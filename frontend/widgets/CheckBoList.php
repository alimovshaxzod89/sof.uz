...
class CheckBoList extends InputWidget
{
    ...
    protected function renderRadioList()
    {
        ...
        foreach ($this->items as $item => $label) {
            $radio   = Html::radio($this->name, false, ['value' => $item, 'id' => $this->attribute . $item, 'class' => 'form-control']);
            $content .= Html::label($radio . $label, $this->attribute . $item, ['class' => 'cb-radio']);
        }
        return Html::tag('div', $content, ['id' => $this->id, 'class' => 'poll__items']);
    }
}