...
<div class="cureency">
    <div class="head_currency">
        <span class="h5"><?= __('Валюта курси:') ?></span> <span class="bank"><?= __('Ўз.Р.Марказий банки') ?></span>
    </div>
    <div class="info">
        <div class="foot_currency">
            ...
            <div class="usd">
                ...
                <div class="block"><span class="micro"><?= __('АҚШ доллари') ?></span> <span class="micro_increase"
                                                                                id="diff_usd"></span>
                </div>
            </div>
            ...
            <div class="euro">
                ...
                <div class="block"><span class="micro"><?= __('Eвро') ?></span><span class="micro_increase" id="diff_eur"></span>
                </div>
            </div>

            <div class="rub">
                <div class="block"><span>RUB</span> <br> <span id="rate_rub" class="course"></span></div>
                <div class="block"><span class="micro"><?= __('Россия рубли') ?></span><span class="micro_increase"
                                                                                id="diff_rub"></span>
                </div>
            </div>
        </div>
            ...
            if (status === 200) {
                ...
                for (const item of birjaData) {
                    if (item.difference >= 0) {
                        birjaStr += `${item.name} <span class="" style="color: green; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif">+ ${item.summa} ${item.difference} ${item.tovarNum} ${item.percent}</span> `
                    } else {
                        birjaStr += `${item.name} <span class="" style="color: red; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif">- ${item.summa} ${item.difference} ${item.tovarNum} ${item.percent}</span> `
                    }
                }
