...
class MP3File
{
    ...
    public static function formatTime($duration) //as hh:mm:ss
    {
        ...
        return $hours ? sprintf("%02d:%02d:%02d", $hours, $minutes, $seconds) : sprintf("%02d:%02d", $minutes, $seconds);
    }
    ...
    public function getWavDuration()
    {
    ...
    public function getAudioDuration($use_cbr_estimate = false)
    {
        if (file_exists($this->filename)) {
            $info = pathinfo($this->filename);
            if (strtolower($info['extension']) == 'wav') {
                return $this->getWavDuration();
            } else {
    ...
    public static function parseFrameHeader($fourbytes)
    {
        ...
        $info['Samples']   = isset($samples[$simple_version]) && isset($samples[$simple_version][$layer]) ? $samples[$simple_version][$layer] : 0;
        return $info;
    }
