...
class LocaleSitemap
{
    ...
    private function startSitemap()
    {
        ...
        $this->getWriter()->writeAttribute('xsi:schemaLocation', 'http://www.sitemaps.org/schemas/sitemap/0.9 http://www.sitemaps.org/schemas/sitemap/0.9/sitemap.xsd http://www.w3.org/1999/xhtml http://www.w3.org/2002/08/xhtml/xhtml1-strict.xsd');
        $this->getWriter()->writeAttribute('xmlns:xhtml', 'http://www.w3.org/1999/xhtml');
        $this->getWriter()->endAttribute();
     ...
     * @return Sitemap
     */
    public function addItem($loc, $priority = self::DEFAULT_PRIORITY, $changefreq = NULL, $lastmod = NULL, $alternate = true)
    {
        ...
        if (strpos($loc, $this->getDomain()) === 0) {
            $url = $loc;
        } else {
        ...
        if ($alternate) {
            foreach (['uz' => 'uz-UZ'] as $slug => $lang) {
                if (strpos($loc, $this->getDomain()) === 0) {
                    $url = $this->getDomain() . $slug . substr($loc, strlen($this->getDomain()) - 1);
                } else {
                    $url = $this->getDomain() . $slug . '/' . $loc;
                }
                ...
                $this->getWriter()->writeAttribute('href', trim($url,'/'));
